...
async function runAction() {
  ...
  const openPullRequests = await octokit.rest.pulls.list({ owner, repo, state: 'open', base: currentRef, per_page: 100 });
  core.info(`Relevant open pull requests: [${openPullRequests.data.map(pr => pr.number).join(', ')}]`);

  // const filteredPullRequests = openPullRequests.data.filter(pr => pr.base.ref === currentRef);
  const prMap = openPullRequests.reduce((acc, pr) => { acc[pr.number] = true; return acc; }, {});
